...
def split_M_files(file_ldscore, chromosome, prefix_genomic_annot, list_annotations):
	...
	message = []
	message.append("CHR={} | Writing .M and .M_5_50 files".format(chromosome))
	...
	message.append("CHR={} | DONE writing .M and .M_5_50 files".format(chromosome))
	return(message)
...
def split_annot_file(file_ldscore, chromosome, prefix_genomic_annot, list_annotations):
	...
	"""
	message = []
	message.append("CHR={} | START splitting annot file".format(chromosome))
	file_ldscore_base = re.sub(r"\.l2\.ldscore\.gz$", "", file_ldscore) # .e.g /scratch/sc-ldsc/nn_lira_sema/nn_lira_sema.COMBINED_ANNOT.9.
	...
	message.append("CHR={} | Read  annot file: {}".format(chromosome, file_annot))
	for i in range(len(list_annotations)):
		if i % 25 == 0:
			message.append("CHR={} #{}/#{}| Writing annot files".format(chromosome, i, len(list_annotations)))
		annotation = list_annotations[i]
	...
	message.append("CHR={} | DONE splitting annot file".format(chromosome))
	return(message)

def split_ldscore_file_per_annotation(file_ldscore):
	message = []
	message.append("Processing file_ldscore {}".format(file_ldscore))
	...
	message = message + split_M_files(file_ldscore, chromosome, prefix_genomic_annot, annotations_clean)
	message = message + split_annot_file(file_ldscore, chromosome, prefix_genomic_annot, annotations_clean)
	for counter, annotation in enumerate(annotations_header):
		if counter % 25 == 0:
			message.append("CHR={} #{}/#{}| Writing ldscore files".format(chromosome, counter, len(annotations_header)))
...
file_ldscore = snakemake.input[0] # e.g. <SOMEPATH>/nn_lira_sema.COMBINED_ANNOT.22.l2.ldscore.gz
...
logfile = os.path.join(out_dir, '{}.{}.log'.format(dataset_prefix, chromosome))
log = split_ldscore_file_per_annotation(file_ldscore)
with open(logfile, "w") as fh_out:
	log_string = '\n'.join(log)
	print(log_string)
	fh_out.write(log_string)
